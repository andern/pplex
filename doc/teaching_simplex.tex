...
In an optimization problem, the objective is to decide on how to utilize given ressources such that they maximize some ``profit'' (or minimize some ``cost''), while satisfying a set of additional constraints on the ressources.
This is commonly done by assoziating the possible decisions with variables, such that the profit and the constraints can be formulated as functions of these variables. 
If the profit (in general called \emph{objective (function)} is linear and the constraints can be formulated as linear (in)equalities, then the optimization problem has a \emph{linear programming (LP)} formulation. An instance of an LP formulation is called \emph{linear program}.
Solving a linear program means identifying those values for the variables which maximize the objective while satisfying all constraints.
...
\[
    \begin{array}{lrcrcrl}
			...
			\text{subject to}&  2 x &+&    y &\leq&   6	& \quad\quad \text{(constraints)}\\
...
%\emph{standard form}: ``Standard form'' is Ax \leq b , Vanderbei p.7
\[    
...
%The tool is further described in Section~\ref{sec:features}.
...
Our contribution \url{pplex},
(\emph{pedagogical implementation of the Simplex method}),
is open-source \cite{pplex} and freely distributed under the 
...
%The standard solution in this case is to change
...
%\subsection{Example for an Introduction into Sensitivity Analysis}

%Once the optimum is found, it is natural to study how depended the optimum is on the available data. 
%As an introduction, the lecturer can ask the students: How much can we fiddle with the coefficients in the objective function without changing the optimal solution? 
%They then can use \texttt{pplex} to experiment with the optimal dictionary, f.ex. of our first example:
...
\subsection{Future extensions}\label{sec:future}
\begin{itemize}
         \item 2D GUI
         \item 3D visualization
	\item We'll make an effort to keep it up to date and will support future standard
...
\begin{figure}[htb]
	\begin{center}
		\includegraphics{ex_gui.jpg}
	\end{center}
	\caption{Example of the Graphical User Unterface}
	\label{fig:ex_gui}
\end{figure}
