...
the dual dictionary are infeasible, solving goes in two phases. In the first phase
we modify the linear program into one which \emph{is} dually feasible.
We do this by changing the objective into one with all coefficients negative.
By solving the modified program we find a feasible solution of the original
program. In the second phase, we solve the original program starting from the 
...
This dictionary is feasible and all coefficients of $y$ are positive. This means that $y$ can increase 
unboundedly, yielding arbitrarily high values for the objective.
...
As an introduction into sensitivity analysis, the lecturer may for example ask the students: How much can we fiddle with the coefficients in the objective function without changing the optimal solution? 
They then can use \texttt{pplex} to experiment with the optimal dictionary, for example of our first example:
\begin{alltt}
\input{nik9.txt}
\end{alltt}
