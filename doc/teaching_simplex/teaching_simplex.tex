...
lines corresponding to a \emph{basic variable being zero} in COLOR. %MB
...
The benefits of \url{pplex} are four-fold: no precious class-room time is wasted
on elementary calculations, no calculation errors distract attention,
the demonstration of larger, more interesting examples becomes feasible, and 
understanding of the simplex method is enhanced by showing the geometric interpretation of two-dimensional problems along executing the simplex method.
...
So far we have only experience with using \url{pplex} in one course with 11 students
...
...
where $x$ and $y$ are two new, non-negative variables. In the geometric interpretation however, one would
