...
portability, 
and that it is free software.
\end{abstract}

\section{Introduction and Motivation}\label{sec:intro}
REMARK FOR 4OR READERS? %MB
...
the objective. Here comes an example:
\begin{alltt}
\input{deg.lps}
\end{alltt}
We note that the lines corresponding to each of the constraints intersect in the point $(2,2)$.
Consider the following sequence of pivots:
\begin{alltt}
\input{deg.txt}
\end{alltt}
Observe that the third pivot above is degenerated and that the objective does not improve.
In the geometrical interpretation, we are in $(2,2)$ after two pivots. This point is obtained
as the intersection of the two lines $w_1 = w_3 = 0$. after the third pivot, we are still
in $(2,2)$, now obtained as the intersection of the two lines $w_2 = w_3 = 0$.
The essential difference between the two situations lies, of course, in their dictionaries.
In order to get a visible change in the graphical illustration, the line corresponding
to a \emph{basic variable being zero} is always depicted in COLOR. %MB

In the above example, using a degenerated pivot could easily be avoided.
Degeneracy would not have occcured if we would have started increasing the
variable $y$, having the largest coefficient. Also, taking as second
textt{pivot 2 2} instead of textt{pivot 2 3} would have avoided degeneracy
(the dictionary would be degenerated, but the pivots would not).
In the classroom, the moment has come to address the general issues
around degeneracy and cycling. 
The classical example \cite{Chvatal,Vanderbei} of the (rare) possibility of 
...
\section{Pedagogical Remarks and Conclusion}\label{sec:pedagogy}
RECALL: The benefits of such a tool are four-fold: no precious class-room time is wasted
on elementary calculations, no calculation errors distract attention,
the demonstration of larger, more interesting examples becomes feasible, and 
understanding of the simplex method is enhanced by showing the geometric interpretation of two-dimensional problems along executing the simplex method.
